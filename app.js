    ...
    try {
        ...
        if (data.status === "error") {
            ...
            document.getElementById("proposal-container").innerHTML =
                `<p style="padding:12px; color:#dc2626;">GASエラー: ${escapeHtml(data.message)}</p>`;
            allOpinions = [];
            return;
        }
        ...
        document.getElementById("proposal-container").innerHTML =
            '<p style="padding:12px; color:#dc2626;">通信エラー。GASのURLとデプロイを確認してください。</p>';
        allOpinions = [];
    }
}
...
function renderProposalTree(opinions) {
    ...
    // 最小構成：ただ出すだけ。CSSクラスも使わない。イベントも付けない。
    let html = '<div style="padding:20px;">';
    html += '<h3>提案一覧：' + opinions.length + '件</h3>';

    for (let i = 0; i < opinions.length; i++) {
        const o = opinions[i];
        html += '<div style="border:1px solid #ccc; margin:5px; padding:10px;">';
        html += (i + 1) + '. ' + (o.title || 'タイトルなし');
        html += '<br><small>' + (o.bigCatName || '') + ' / ' + (o.midCatName || '') + '</small>';
        html += '</div>';
    }
    html += '</div>';

    container.innerHTML = html;
    console.log('最小構成で描画完了');
}
function clearForm() {
